...
impl NetworkGraph {
    ...
    fn draw_packet_loss_bars(&self, x: f32, y: f32) {
        ...
        for metrics in self.metrics_history.iter() {
            if metrics.packet_loss_percent > 0.1 {
                ...
                let time_offset = metrics
                    .timestamp
                    .duration_since(oldest_timestamp)
                    .as_millis() as f32;
                let bar_x = x + (time_offset / time_span_ms) * usable_width;
                let bar_height = (metrics.packet_loss_percent / 10.0 * usable_height * 0.3)
                    .min(usable_height * 0.3);
    ...
    fn draw_labels(&self, x: f32, y: f32) {
        ...
        if let Some(latest) = self.metrics_history.back() {
            let current_info = format!(
                ...
                latest.ping_ms, latest.packet_loss_percent, latest.jitter_ms
            );
    ...
    fn draw_legend(&self, x: f32, y: f32) {
        ...
        draw_text(
            "Line = Ping | Red bars = Packet loss",
            x,
            explanation_y,
            10.0,
            Color::from_rgba(180, 180, 180, 255),
